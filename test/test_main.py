...
class TestShape1D(unittest.TestCase):
    def setUp(self):
        # Minimal dummy data for Fem1D
        x = torch.tensor([0.0, 1.0, 2.0])
        conn = torch.tensor([[1, 2, 3]])
        mat = MaterialProperties(E=torch.tensor([[1.0]]), area=torch.tensor([[1.0]]), b=0.0)
        bc = BoundaryConditions(u_d=torch.tensor([0.0]), drlt_dofs=torch.tensor([1]), f_sur=torch.zeros(3))
        self.fem = Fem1D(x, conn, mat, bc)
    ...
    def test_sum_of_shape_functions_is_one(self):
        N, _ = self.fem.shape1d(self.xi_test, self.nen)
    ...
    def test_sum_of_shape_function_derivatives_is_zero(self):
        _, gamma = self.fem.shape1d(self.xi_test, self.nen)
...
class TestGauss1D(unittest.TestCase):
    def setUp(self):
        ...
        conn = torch.tensor([[1, 2]])
        mat = MaterialProperties(E=torch.tensor([[1.0]]), area=torch.tensor([[1.0]]), b=0.0)
        bc = BoundaryConditions(u_d=torch.tensor([0.0]), drlt_dofs=torch.tensor([1]), f_sur=torch.zeros(2))
        self.fem = Fem1D(x, conn, mat, bc)

    def test_nqp_1(self):
        xi, w8 = self.fem.gauss1d(1)
...
class TestJacobian1D(unittest.TestCase):
    ...
    def test_linear_element(self):
        xe = torch.tensor([2.0, 4.0])
        gamma = torch.tensor([-0.5, 0.5])
        detJq, invJq = self.fem.jacobian1d(xe, gamma, nen=2)
    ...
    def test_quadratic_element(self):
        ...
        detJq, invJq = self.fem.jacobian1d(xe, gamma, nen=3)
